...
import Lang.Frontend.Semantics.CommandTypeChecker (tcProgram)
    ...
    case flag opts of
        Typed ->
          case tcProgram ast of
            Left err -> print err
            Right _ -> interpreter ast
        Untyped -> interpreter ast
...
buildOption theFlag theFile = case theFlag of
    "--typed" -> return $ Right $ Option Typed theFile
    "--untyped" -> return $ Right $ Option Untyped theFile
    _ -> errorMessage  
